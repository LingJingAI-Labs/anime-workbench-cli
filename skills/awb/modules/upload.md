...
- **主体素材**（`subject-publish` / `subject-upload`，需有权限账号）：Seedance 2.0 主体参考链路专用。多张参考图一起注册为一个"主体"，拿到 `subjectId`（格式 `asset-xxx`），后续靠 `@角色名` + `--refSubjects "角色=asset-..."` 稳定引用。默认推荐 `subject-publish`，因为它会优先用安全代码名生成素材组名/素材名，降低敏感词卡审核的概率。
- **Files 后缀 vs 无 Files**：创作命令里
  - `--irefFiles` / `--crefFiles` / `--srefFiles`：本地文件，CLI **自动上传**后引用
  - `--iref` / `--cref` / `--sref`：已经在素材桶里的 backendPath，直接复用
- **`upload-files.sceneType`**：决定素材归类（`material-image-draw` 生图池 / `material-video-create` 生视频池）。默认 `material-image-draw`。做参考生视频时，优先先传到 `material-video-create` 再复用 backendPath。
...
| `subject-publish` / `subject-upload`（**需有权限账号**） | 把角色 / 人物注册成 Seedance 2.0 可复用主体 | 必读 [`../references/subject-upload.md`](../references/subject-upload.md)；返回"引用写法"列可直接粘到 `--refSubjects` |
...
# Seedance 2.0 真人主体参考生视频：先把 backendPath 注册成主体 asset id
...
- **Seedance 2.0 真人主体优先 `subject-publish`**：如果你的目标是拿 `asset-xxx` 去 Seedance 2.0 的 `--refSubjects`，就不要停在 `upload-files`；已有 backendPath 时继续走 `subject-publish --primaryUrl <backendPath>`。默认会用安全代码名做素材组名/素材名；想自定义时传 `--publishCode`。其他普通参考模型继续走 `--refImageFiles` / `--refImageUrls`，不要说成主体加白。
- **真人短剧 / 多段复用必须先讲两条路径**：用户说“做剧”“真人图”“同一角色多镜头”时，若选 Seedance 2.0，优先建议上传到生视频池后 `subject-publish` 成主体（平台加白 / 可引用资产），再在视频里用 `--refSubjects`。如果只是一次性试片、账号无主体权限、选的是普通参考模型、或用户明确要直接传 webp，才退回 `--refImageFiles "角色=./person.webp"`。
...
- **Seedance 批量加白/主体注册优先 `subject-publish-batch`**：对多个人物一次性跑完，结果里直接拿 `引用写法` 回填到后续 Seedance 任务，最适合 agent 和会断线的沙箱。
