...
**不要混用**：首尾帧模式的 `frame*` 和参考生视频模式的 `ref*` 不能同时出现。
...
- **`frame*` vs `ref*` 不共存**：选了首尾帧路径就别再塞 `ref*`；选了参考生视频就别再塞 `frame*`。想更底层的控制去 `--framesJson` / `--promptParamsJson`。
- **`@名称` 要和命名参考对得上**：
...
- **`--generatedTime` 单位是秒**：常见 5 / 10；部分模型只接离散值。
- **Token 计费模型（Seedance 2.0 系列）先 `video-fee`**：和按张 / 按次计费的不一样，提示词长度 / 参考数量都会推高成本。
- **故事板、音效开关不是所有模型都有**：查 `video-models` 的 `特色能力` 列；也能在 `paramKeys` 里看 `multi_prompt` / `audio` 是否存在。
